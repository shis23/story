...
/// `current_exe().parent()` 的等价输入，用于 exe_dir/data 回退。
fn resolve_app_data_dir(
    ...
    let os_dir = match target {
        AppDataDirTarget::Android => {
            ...
            //
            // 初始化顺序约束（已核实 tauri 2.11.2 源码）：`get_app_data_dir()` 在
            // `run()` 最开始被调用，早于 `tauri::Builder::default()` 与 `.setup()`；
            // 而 `app.path().app_data_dir()` 在 Android 上经 PathResolver→
            // PluginHandle→AppHandle→Kotlin PathPlugin(JNI，需运行中的 Activity)
            // 解析，Builder 构造前完全不可用，Tauri 也不设置任何环境变量。
            // 因此此处唯一可行的设计是：可覆盖环境变量 + 应用私有 files 目录常量。
            //
            // 真机冒烟/特殊 ROM/多用户：用 `STORYFORGE_DATA_DIR` 覆盖。
            // 长期演进路径见 RESULT「Android 数据目录注入门」。
            env("STORYFORGE_DATA_DIR")
                .map(PathBuf::from)
                .or_else(|| Some(PathBuf::from("/data/data/com.storyforge.app/files")))
        }
        AppDataDirTarget::Windows => env("APPDATA").map(|appdata| PathBuf::from(appdata).join("StoryForge")),
        AppDataDirTarget::Macos => env("HOME").map(|home| PathBuf::from(home).join("Library/Application Support/StoryForge")),
...
///
/// Android：返回应用私有 files 目录 `/data/data/com.storyforge.app/files`
/// （`STORYFORGE_DATA_DIR` 可覆盖）。注意 Tauri 的 `app.path().app_data_dir()`
/// 在 Android 上返回 `Context.getDataDir()`（即 `/data/data/com.storyforge.app`，
/// 不含 `/files`），与本目录为父子关系；二者都指向同一应用沙箱，单用户安装下
/// 状态收敛。Builder 构造前无法用框架 API 解析，故用约定路径（详见
/// `resolve_app_data_dir` 的顺序约束说明）。
...
mod tests {
    ...
    fn resolve_app_data_dir_returns_os_standard_paths_per_platform() {
        ...
        assert_eq!(lin_home, PathBuf::from("/home/test/.local/share/storyforge"));
    }

    /// AND-2 Android 路径解析契约：
    /// - `STORYFORGE_DATA_DIR` 覆盖优先（真机冒烟/调试/特殊 ROM）；
    /// - 无覆盖时落到应用私有 files 目录（只读 APK 与无意义 XDG 之外的可写沙箱）。
    #[test]
    fn resolve_app_data_dir_android_prefers_env_override_then_sandbox_files() {
        ...
        let default = resolve_app_data_dir(AppDataDirTarget::Android, &env_view(&[]), None);
        assert_eq!(
            default,
            PathBuf::from("/data/data/com.storyforge.app/files"),
            "Android 默认应落到应用私有 files 目录（非只读 APK、非无意义 XDG）"
        );
    }
    ...
    fn migrate_from_exe_dir_copies_when_new_dir_empty_and_skips_when_populated() {
        ...
        std::fs::write(old_dir.join("characters.json"), b"{}")
            .expect("seed old data");
        ...
        std::fs::write(old_dir.join("characters.json"), b"\"stale\"")
            .expect("rewrite old data");
        migrate_from_exe_dir_if_needed(&new_dir, Some(&exe_parent));
