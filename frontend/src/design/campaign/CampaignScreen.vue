 ...
 * 对齐 selected 图④：
 *   - 左：我的活动列表
 *   - 右：当前活动详情 + 实例/知识/任务/总结 tabs
 *   - 下方：最近实例 / 知识库概览卡
 *
 * 不 import store/tauri；业务数据与操作全部 props/events。
 * 实例 MVU 条等能力敏感区通过 instanceExtra slot / prop 组件注入。
 */
...
const props = defineProps({
  ...
  detailTab: { type: String, default: 'instances' }, // instances|knowledge|tasks|summaries
  ...
  <div class="h-full flex flex-col bg-bg">
    ...
    <header class="shrink-0 h-14 px-4 sm:px-6 border-b border-line bg-surface flex items-center gap-3">
      <div class="min-w-0 flex-1">
        ...
        <h1 class="text-sm font-semibold text-ink truncate">{{ title }}</h1>
      </div>
      <button
        type="button"
      ...
      <main class="flex-1 min-w-0 overflow-y-auto">
        <div v-if="!selectedCampaign" class="h-full flex items-center justify-center p-8 text-center">
          <div>
            ...
            <button
              type="button"
              class="mt-5 min-h-9 px-4 rounded-md bg-accent text-white text-[13px] font-medium hover:bg-accent-bright transition-colors"
              @click="emit('new-campaign')"
            >新建活动</button>
          </div>
        </div>
        ...
        <div v-else class="p-4 sm:p-6 space-y-5">
          ...
          <section v-else-if="detailTab === 'instances'">
            <div v-if="!instances.length" class="rounded-xl border border-line bg-surface px-5 py-10 text-center text-sm text-ink-soft">
              还没有角色实例
            </div>
            <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <article
                ...
                :key="inst.id"
                class="rounded-xl border border-line bg-surface shadow-card p-4"
              >
                ...
                <div v-if="inst.summary" class="mt-3 text-xs text-ink-soft line-clamp-3 leading-relaxed">{{ inst.summary }}</div>
              </article>
            </div>
          </section>
          ...
          <section v-else-if="detailTab === 'tasks'">
            ...
            <ul v-else class="space-y-2">
              <li
                v-for="t in tasks"
                :key="t.id || t.title"
                class="rounded-xl border border-line bg-surface shadow-card px-4 py-3 flex items-start gap-3"
              >
          ...
          <section v-else>
            ...
            <ul v-else class="space-y-2">
              <li
                ...
                :key="s.id || s.round || s.title"
                class="rounded-xl border border-line bg-surface shadow-card px-4 py-3"
              >
