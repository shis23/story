...
function bootstrapSrcdoc() {
  ...
  const lines = [
    ...
    "    }",
    "    // Zod v4 (prefault/loose) via ESM bundle — MagVarUpdate/data_schema/mvu_zod require v4 APIs.",
    "    if (!window.__sfZodV4Ready) {",
    "      var zodUrl = 'https://testingcf.jsdelivr.net/npm/zod@4.4.3/+esm';",
    "      try {",
    "        var zodMod = await window.__sfThImportUrl(zodUrl);",
    "        var api = (zodMod && (zodMod.z || zodMod.default)) || zodMod;",
    "        if (!api || typeof api.object !== 'function') {",
    "          api = zodMod;",
    "        }",
    "        if (!api || typeof api.object !== 'function') throw new Error('Zod v4 module missing .object');",
    "        // MagVarUpdate: const ue=z; ue.object(...)",
    "        // data_schema/mvu_zod: const t=z; t.z.object(...)",
    "        window.z = api;",
    "        if (!window.z.z) window.z.z = api;",
    "        window.Zod = window.z;",
    "        window.__sfZodV4Ready = true;",
    "      } catch (e) {",
    "        console.warn('[TH] Zod v4 ESM load failed, trying classic UMD v3 fallback', e);",
    "        if (!window.Zod) {",
    ...
    "        if (window.z && !window.z.z && window.z.object) window.z.z = window.z;",
    "      }",
    "    }",
