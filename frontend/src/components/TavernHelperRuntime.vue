...
function bootstrapSrcdoc() {
  ...
  const lines = [
    ...
    "      // MagVarUpdate: const ue=z; ue.object(...).loose().prefault()",
    "      // data_schema/mvu_zod: const t=z; t.z.string(...).prefault()",
    "      window.z = api;",
    "      window.z.z = api;",
    "      window.Zod = api;",
    "      try { globalThis.z = api; globalThis.Zod = api; } catch (e0) {}",
    "      window.__sfZodV4Ready = true;",
    "      console.info('[TH] Zod v4 ready', typeof api.object, typeof api.string().prefault, typeof api.object({}).loose);",
    "    }",
