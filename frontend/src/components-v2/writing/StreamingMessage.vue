<script setup>
/**
 * StreamingMessage — 写作过程流式消息（v2 迁移）
 *
 ...
 *
 * 全程读 writingStore.pipeline 状态机（由父级作为 pipeline prop 注入），自身无独立状态。
 * draft_ready 后由 useWriting.applyConversation 推入正式成文，本组件随 showPipeline=false 消失。
 */
