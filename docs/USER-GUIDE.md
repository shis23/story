...
> **文档状态**：2026-07-08 初版，2026-09-05 修订（v3 故事快照与已采纳历史保护）。本文面向首次使用者，覆盖使用前准备、导入 ST PNG/JSON、创建 Campaign、写第一轮、查看状态、失败排障、导出、兼容/降级边界、本地数据与备份。
>
...
- 升级前备份：关闭应用后保留完整应用数据目录的独立副本，再导出 Campaign 故事快照。排障 bundle 只用于定位问题，不是数据备份。

### 界面配色

展开左侧栏，在底部“配色”中选择青绿、蓝灰、玫红或经典，整套界面即时切换。
“经典”恢复原版暖米白纸面与金棕强调色，夜读时使用原版暖深棕与浅金色。
“夜读模式”独立控制浅色与深色，不会重置所选配色。选择保存在当前设备，
重开应用仍会恢复；不会改变故事内容或清空尚未发送的写作意图。
键盘可用 Tab 聚焦色块，再用方向键选择。
...
- Campaign bundle：v3 是单局故事快照，包含正文及变体、终态轮次、源角色卡、本局世界书、实例、变量、知识、任务和总结。旧 v1/v2 只是状态交换包，不含正文及轮次，不能恢复完整故事。
- 排障 bundle（界面中在 LogPanel 点击 📦 或 "导出 bundle" 按钮触发）：用于反馈失败的诊断包，通常包含 app/platform 摘要、数据目录/日志/会话路径摘要和关键 store 摘要；它不应包含真实 API key。
- StoryForge JSON：不等同于原始 ST 卡。故事快照不包含连接凭据、插件安装、预设、Meta 工作区、MVU 翻译缓存及外部资源文件；向量索引导入后重建。它不是完整应用备份，迁移时仍需保留原数据目录和外部资产。
...
请先采纳或放弃当前轮次，再导出。v3 导入会为消息、轮次及故事对象分配新 ID，保留正文与状态，不覆盖旧局。旧版包导入后会明确提示“不能恢复完整故事”；不能因为导入成功就删除原设备数据。

分支目前只支持已采纳的末尾正文，会继承正文、实例、变量、知识、任务、总结和本局世界书。尚不支持任意历史节点分支或已采纳历史回滚。删除未采纳稿件会删除其所属整轮及后续未采纳内容，并放弃对应轮次；已采纳历史及其之前的用户意图不能直接编辑、删除、新增或切换变体。
...
StoryForge 以本地数据为主。应用不应被视为云同步或唯一备份来源；重要故事应同时保留 v3 故事快照和完整应用数据的独立副本。
