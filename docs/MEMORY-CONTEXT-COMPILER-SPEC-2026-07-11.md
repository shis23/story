...
| 基线 | `a338cfb` + S3 实体/标识符匹配小修 |
| endpoint host | `cli.2529985.xyz` |
| model | `grok-4.5` |
| 结论等级 | **PROBE EXECUTION PASS；M5 ACCEPTANCE: Partial Evidence**（非完整参数标定；非生产 Accept；S3 非语义保真） |
| 证据可审计性 | 仓库无脱敏 JSONL；下列 cache 数值 / 4/4 / 6/6 为提交者运行记录，**不可从仓库独立重放审计** |

| Suite | 重跑结论 | 要点 |
| --- | --- | --- |
| **S1** | 生成 **Pass**；写作 cache **有信号**（解析链路有效，≠ 缓存已充分优化） | 仅 `turn*`；early_max_cached=2944 → late=4224；`any_stream_write_cached=true`；最大 prompt system_hash 六轮一致（身份仍非严格 Director） |
| **S2** | 未重跑 | 仍以工具目录 Pass / Director 召回未证为准 |
| **S3** | 分组/covers/旧 A **Pass**；B-only **实体/标识符保留 6/6** | 仅扫 parents；token 存在：**不是**否定极性 / 有向因果 / 限定词语义保真。核：`卫岚澈`/`A7F2`/`03:17`/`走私`/`口令+伏击`/`31.208` |
