...
- **Regex World Info 作用域**：当前 ST placement 5 已接到常驻/触发世界书注入和 `search_world_info` 工具返回；placement 1/2 已分别接到用户输入与 AI 输出。Slash placement 3 与 Reasoning placement 6 已被执行器识别，但完整 app hook 仍待接。
