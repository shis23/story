...
- **Regex display-only 渲染**：`infra-regex` 已区分 Prompt/Persisted/Display 目标，`promptOnly` 不再改显示/存储、`markdownOnly` 不再污染 prompt/持久化；但 Display 目标尚未接到消息展示或 PluginHost 渲染层。
