...
- **ST 事件总线覆盖度**：写作与重 roll 的主 `PipelineEvent` 已通过 `App.vue` → `DebugDrawer` → `PluginHost` → iframe 发送给插件；插件可订阅 `pipeline.<event_type>`、原始事件名，以及 `GENERATION_STARTED` / `STREAM_TOKEN` / `GENERATION_ENDED` / `MESSAGE_RECEIVED` 等常用别名。Plugin iframe 已注入 ST 风格 `event_types` / `eventTypes` 与 `eventSource.on/once/makeFirst/makeLast/removeListener/emit` shim；ST 99 事件全集的真实后端 emit 和 prompt 组装钩子仍待补。
