...
- **Regex World Info/Reasoning 作用域**：当前 ST placement 5 已接到常驻/触发世界书注入和 `search_world_info` 工具返回；placement 6 已接到 AI 输出中的 `<think>` / `<thinking>` 推理块，持久化路径会在落盘前处理，`markdownOnly` 展示路径会只派生 `display_content`。placement 1/2 已分别接到用户输入与 AI 输出；Slash placement 3 已被执行器识别，但真实斜杠命令输入 hook 仍待接。
