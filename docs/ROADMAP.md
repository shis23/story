...
- **Regex World Info/Reasoning/Slash 作用域**：当前 ST placement 5 已接到常驻/触发世界书注入和 `search_world_info` 工具返回；placement 6 已接到 AI 输出中的 `<think>` / `<thinking>` 推理块，持久化路径会在落盘前处理，`markdownOnly` 展示路径会只派生 `display_content`。placement 1/2 已分别接到用户输入与 AI 输出；Slash placement 3 已在用户/导演意图以 `/` 开头时运行，并会在随后继续执行 Input 正则。完整 Slash 命令注册、参数管道和 PluginHost/JS 斜杠运行时仍待后续兼容。
...
- **ST 事件总线覆盖度**：写作与重 roll 的主 `PipelineEvent` 已通过 `App.vue` → `DebugDrawer` → `PluginHost` → iframe 发送给插件；插件可订阅 `pipeline.<event_type>`、原始事件名，以及 `GENERATION_STARTED` / `STREAM_TOKEN` / `GENERATION_ENDED` / `MESSAGE_RECEIVED` 等常用别名。Plugin iframe 已注入 ST 风格 `event_types` / `eventTypes` 与 `eventSource.on/once/makeFirst/makeLast/removeListener/emit/emitAndWait` shim，`emit` 会顺序等待 async listener 并支持返回新 payload 串联；普通事件 feed 已按 `event_subscriptions` 过滤，并在无 `ReadMemory` 时移除正文类字段。主聊天宿主动作已开始 emit `APP_READY`、`CHAT_LOADED`、`CHAT_CHANGED`、`MESSAGE_SENT/RECEIVED/UPDATED/DELETED/SWIPED`、`CHARACTER_LOADED`。写作前 intent/prompt 级 hook 已通过常驻隐藏 `PluginHost` 触发 `GENERATE_BEFORE_COMBINE_PROMPTS` / `CHAT_COMPLETION_PROMPT_READY` 并按声明 `ModifyPrompt` 的插件顺序合并；最终 LLM messages 级 hook 已通过后端 `prompt_hook_request` 事件和 `plugin_prompt_hook_result` 回传接入 `start_writing` / `regenerate`，超时或插件错误 fail-open；prompt hook 基础脱敏审计记录已补。ST 99 事件全集的真实后端 emit、冷门 Slash/TavernHelper 语义和完整审计 UI/导出仍待补。
