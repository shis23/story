...
- ❌ 早期判断"无事件总线" → ✅ 实际前端已有事件分发；2026-07-07 已把主生成链 `PipelineEvent` 桥到插件 iframe，并注入 ST 风格 `eventSource`/`event_types` shim（含 async `emit` 与 `emitAndWait`）；缺的是 ST 全量事件真实 emit 与 prompt hooks
