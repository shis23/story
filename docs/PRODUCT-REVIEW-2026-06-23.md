...
| regex_scripts | 🟡 局部运行时接入 | `infra-regex` + `WritingContext.regex_scripts`；active Preset + legacy 选中角色卡 Scoped 脚本已跑 Input/Output |
...
| # | 缺口 | 现状 | 工作量 |
|---|---|---|---|
| R | **正则系统（独立工作项，见 §2.3.1）** | Preset + Scoped 来源已可 typed 读取并可按 ST 顺序合并；active Preset 已可在预设面板启用并进入写作/重 roll；执行器已支持 `/pattern/flags`；流水线已执行 `WritingContext.regex_scripts` 的 Input/Output；Global 运行时装配和多作用域仍未完成 | **4-8 天** |
...
| **Scoped 脚本（卡内）** | 角色卡 `data.extensions.regex_scripts` | 🟡 legacy 选中卡已执行 Input/Output | `Character::scoped_regex_scripts()` 已可解析；Tauri legacy 写作从选中卡收集并注入 `WritingContext.regex_scripts`；Campaign 活动卡装配待补 |
...
| 子项 | 工作量 |
|---|---|
| 来源合并（Global/Preset/Scoped + 优先级）| 🟡 Preset/Scoped 合并 helper 已完成；active Preset 与 legacy Scoped 运行时已接；Global 配置读取与运行时装配待接 |
