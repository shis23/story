...
| 命定之诗 | 核心 10 项全通 + 5 项降级 | + ST 兼容层（TavernHelper shim + CDN 白名单 + script.js 桩） | 见 §3.5 |
...
#### S2.5：ST 兼容层（10-15 天，新增）
- TavernHelper shim（38 方法转发到 window.storyforge）
- CDN 白名单 + 用户授权 UI
- script.js 最小桩模块（给 LWB，需逆向 LWB 实际依赖）
...
**总计**：约 59-87 天（S1-S6 累加）。**Android MVP（S5 结束）：约 49-67 天**
...
**诚实限制**：
- LWB 静态 `import script.js` 需逆向（可做，几十行桩模块）
...
- 重写 JS-Slash-Runner（实现它依赖的 shim 而非重写它）
