...
| H-014 | 同步 fs 阻塞 tokio | 🟡 部分完成 | 🟡 中 | 2026-07-07 已把 `start_writing` / `regenerate` 的 Campaign 快照读取移入 `fill_campaign_context_async` + `spawn_blocking`，把成功写作/重 roll 后的临时 instance 落盘移入 `persist_temporary_instances_async`，把 postprocess summary/knowledge/variable/task 写回移入 `persist_postprocess_outcome_async`，并把对话变体采纳写盘移入 `accept_variant_async`；其他 store 同步 I/O 仍待分批 |
...
- H-014 同步 fs → `spawn_blocking`（写作/重 roll Campaign 快照读取、临时 instance 落盘、postprocess 写回、accept variant 写盘已完成，其他 store 分批推进）
