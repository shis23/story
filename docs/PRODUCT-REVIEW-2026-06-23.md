...
| # | 缺口 | 现状 | 工作量 |
|---|---|---|---|
| R | **正则系统（独立工作项，见 §2.3.1）** | Global + Preset + Scoped 来源已可 typed 读取并可按 ST 顺序合并；Global settings JSON 导入命令、active Preset、legacy 选中卡和 Campaign 活动卡 Scoped 已进入运行时；执行器已支持 `/pattern/flags`；流水线已执行 `WritingContext.regex_scripts` 的 Input/Output/World Info/Reasoning/Slash 最小路径，且已按当前 ST `placement_codes`（1=User Input，2=AI Output，3=Slash Commands，5=World Info，6=Reasoning）接入；Slash placement 3 会在 `/` 前缀用户/导演意图上先于 Input 正则执行；Reasoning 会处理 AI 输出中的 `<think>` / `<thinking>` 块；`promptOnly` 已只进提示词路径，`markdownOnly` 不再污染持久化文本；消息列表 display-only 文本和派生 HTML 片段渲染已接；常用 Slash 命令注册/触发 shim 与 PluginHost/JS 状态栏 per-slot 挂载已完成，完整 ST prompt hook 参数管道和冷门事件仍未完成 | **0.5 天** |
...
| Slash Commands | 3 | 🟡 最小输入 hook + 常用 shim 已接 | `/` 前缀用户/导演意图会执行 Slash 正则；插件桥已提供常用 Slash 命令注册/触发 fallback，冷门参数管道和完整 ST JS runtime 仍待真实插件回归 |
| **World Info** | 5 | ✅ Prompt | 世界书内容注入前格式化已接入，不改写原始条目 |
| Reasoning | 6 | 🟡 `<think>` 块 hook | AI 输出落盘前和消息 `display_content` 派生展示都会处理 `<think>` / `<thinking>` 推理块 |
...
| Only Display | 只改显示不改存储 | 🟡 执行器已有 Display 目标，且 `markdownOnly` 不再改 prompt/存储；消息列表文本和派生 HTML 片段展示通道已接，PluginHost/JS 状态栏 per-slot 挂载已接 | **展示替换可派生文本或安全 HTML 片段，且不会破坏原 `<data_block>` 存储；复杂脚本状态栏仍需真实卡 WebView 回归** |
...
| 作用域扩展（加 World Info/Slash/Reasoning）| 🟡 当前 ST placement 常量已对齐；User Input `[1]`、AI Output `[2]`、Slash `[3]` 最小 `/` 前缀输入 hook、World Info `[5]`、Reasoning `[6]` 的 `<think>` / `<thinking>` 块已接入；常用 Slash 命令注册/触发 shim 已补，冷门 ST slash 参数管道仍待真实插件回归 |
| 瞬时性（Display-only/Prompt-only/Both）| 🟡 Prompt/Persisted/Display 目标分流已接入；消息列表 display-only 文本和派生 HTML 片段渲染已接，PluginHost/JS 状态栏 per-slot 挂载已接，复杂状态栏脚本仍需真实卡回归 |
...
| H-013 | CampaignStore 集合级锁 + 同步 JSON I/O | 🟡 部分完成 | 🟡 中 | S4（单 Mutex 已拆；写作/重 roll 快照读取、临时 instance 落盘、postprocess 写回已 `spawn_blocking`；剩余同步写入压测/后台 flush 待做） |
| H-014 | 同步 fs 阻塞 tokio | 🟡 部分完成 | 🟡 中 | 2026-07-07 已把 `start_writing` / `regenerate` 的 Campaign 快照读取移入 `fill_campaign_context_async` + `spawn_blocking`，把 `start_writing` 对话准备/legacy 开场白/user intent 写入移入 `prepare_start_conversation_async`，把成功写作/重 roll 后的临时 instance 落盘移入 `persist_temporary_instances_async`，把 postprocess summary/knowledge/variable/task 写回移入 `persist_postprocess_outcome_async`，把对话变体采纳写盘移入 `accept_variant_async`，把嵌入配置写盘移入 `configure_embedder_async`，把归档消息读取移入 `archivable_messages_async`，并把连接 active-state 写盘、MVU 翻译保存、角色识别卡保存、inspect_generation 溯源读取、meta_classify_st_preset 预设读取移入 blocking pool；其他 store 同步 I/O 仍待分批 |
...
- H-013 CampaignStore 写入性能压测；集合级锁、写作快照读取 offload、临时 instance 落盘 offload、postprocess 写回 offload 已完成，必要时继续做后台 flush / 写入异步化
- H-014 同步 fs → `spawn_blocking`（写作/重 roll Campaign 快照读取、start_writing 对话准备、临时 instance 落盘、postprocess 写回、accept variant 写盘、configure embedder 写盘、归档消息读取、连接 active-state 写盘、MVU 翻译保存、角色识别卡保存、inspect_generation 溯源读取、meta_classify_st_preset 预设读取已完成，其他 store 分批推进）
