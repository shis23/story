...
- 前置拆分提交：`cb79875 refactor(tauri): tighten command module boundaries`；本次返修提交：`0485d98 fix(storage): make playthrough deletion retryable`、`c5be471 fix(storage): satisfy rollback lint gate`、`d0acea2 fix(storage): close deletion consistency races`、`c10c9b5 fix(campaign): serialize active selection updates`、`d552344 test(campaign): make active update race deterministic`。
