...
> 状态：**Gate 0 已完成；Gate 1 第一至第三批及世界书/变量子批完成，后续批次待执行**。本文件只建立执行顺序、边界和验收门槛，不代表后续阶段已经完成。
> 起草基线：`main@2832030`。
...
- `crates/tauri-app/src/lib.rs`：约 **19,776 行**（Gate 1 世界书/变量子批后的实时基线）。
...
4. world-info、variables（已完成，拆成两个独立子批）；MVU、Meta 待执行；
5. campaigns、turns；
