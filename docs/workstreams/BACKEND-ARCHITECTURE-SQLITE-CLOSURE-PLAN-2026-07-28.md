...
5. inline tests 跟随被测域迁移（writing/conversations 纯测试已归位，其余按被测域继续）；
...
- 代码提交：`0485d98 fix(storage): make playthrough deletion retryable`、`c5be471 fix(storage): satisfy rollback lint gate`、`d0acea2 fix(storage): close deletion consistency races`。
