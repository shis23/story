...
> 状态：**Gate 0 已完成；Gate 1 第一至第三批、世界书/变量、MVU runtime、Meta Agent、typed patch/MVU 和 Campaign 子批完成，后续批次待执行**。本文件只建立执行顺序、边界和验收门槛，不代表后续阶段已经完成。
> 起草基线：`main@2832030`。
...
- `crates/tauri-app/src/lib.rs`：约 **17,373 行**（Gate 1 Campaign 子批后的实时基线）。
...
4. world-info、variables、MVU runtime、Meta Agent、typed patch/MVU、Campaign（已完成，拆成独立子批）；turns、writing 待执行；
5. campaigns、turns；
