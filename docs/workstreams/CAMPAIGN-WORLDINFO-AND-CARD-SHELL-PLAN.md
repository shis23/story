...
> 状态：执行中（2026-07-22）
>
> **进度**：Phase 0–2 已落地（活动世界书存储/UI、卡只读、Meta 写活动书）。Phase 3–4 已落地提取器 + allowlist 缓存 + 可见 `CardShellHost`（写作屏 #shell 挂 status/opening）。Phase 5 消息 display 分流（`ShellAwareContent` + `extractShellMountsFromDisplay`）已接线写作正文。Phase 6 TH 顺序自动执行（`TavernHelperRuntime` 宿主代持 ES module 图 + inline 工坊脚本，写作屏状态条可重跑）已落地。Phase 7：变量出站落盘 campaign/instance + TH 可见按钮 + `scripts/run-card-shell-evidence.ps1` 证据套件已落地（GUI 截图仍建议手工补）。  
