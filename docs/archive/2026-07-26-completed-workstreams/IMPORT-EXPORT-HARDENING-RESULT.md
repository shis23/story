...
2. **已修损失**：
   - UTF-8 BOM JSON 与 PNG `chara` 内嵌 JSON 均可导入；
   - `character_book` 顶层 metadata（name/description/scan_depth/extensions 等）经 `WorldInfoBook.metadata` 保留并写回；
   - world-info entry 未知字段（如 `probability` / `automation_id`）经 entry `extra` 保留并写回；
   - 旧版 `key` / `keysecondary` 别名解析，并在**首次导入**规范化进 `raw_card_json`；
   - Campaign Bundle 写入前严格读取所有目标 collection 并与内存快照比对；预存坏 JSON / 非法路径会在任何写入前 fail-closed；
   - 中途写失败后补偿回滚，并以严格磁盘快照复核，不吞掉回滚失败；Conversation create 也改为“先持久化、后入缓存”；
   - Bundle 内坏 definition / knowledge source / task related_characters / summary covers 引用 **fail-closed**，不再静默丢弃；
   - Chronicle A/B/C：导入前验证 scope、lineage、code/level、连续 span、双向 covers 与无环图；随后重写 `covers`/`covered_by`，并用 id-keyed insert 避免 B/C 被 A 按 turn 覆盖；源 store 的 pending publication marker 不跨 Bundle 携带。
...
| ST PNG tEXt `chara` | `import_character_from_png` / `write_st_card_png` | 保真 | 内嵌 JSON 支持 BOM；CRC/截断/坏 base64/坏 JSON fail-closed |
| UTF-8 BOM JSON | `strip_utf8_bom` | 已修 | 裸 JSON 与 PNG 内嵌 JSON 的 BOM 前缀均不再 JsonError |
...
| World book entry extras | `WorldInfoEntry.extra` | 已修 | entry-level 未知字段 round-trip |
...
| Campaign JSON Bundle v2 | export/import | 加强 | strict preflight + 校验引用 + 磁盘验证回滚 + 变量/多角色/知识/Chronicle 图 |
...
| PNG 内嵌 BOM JSON 导入失败 | `test_import_character_from_png_accepts_bom_json_payload` 曾 `JsonError` | base64 decode 后复用 `strip_utf8_bom` |
...
| world-info entry 未知字段首导丢失 | `test_world_info_entry_unknown_fields_survive_first_import_and_export` | `StWorldInfoEntry` flatten extra + domain entry extra |
...
| Conversation 持久化失败仍污染缓存 | `create_persisted_fails_without_polluting_cache` | import 使用 persist-before-cache create |
| Bundle 中途失败 partial write | `import_campaign_bundle_is_atomic_on_mid_write_failure` / `..._rollback_is_verified_on_disk` | 补偿回滚 + 严格磁盘快照验证 |
| 预存损坏 collection 被 fail-open loader 当空集合覆盖 | `import_campaign_bundle_rejects_preexisting_corrupt_store_before_any_write` | 写入前 strict disk read + memory/disk baseline equality |
...
| 畸形 Chronicle 图可写入 | `import_campaign_bundle_rejects_malformed_summary_graphs_before_writes` | scope/lineage/code/span/edge/cycle fail-closed validator |
...
- legacy Chronicle summary 可没有 `code`；一旦存在 code，就必须可解析、与 A/B/C level 一致且在该 lineage 的 Bundle 内唯一。
- 源 Campaign 的 `pending_compress_publication` 不随 Bundle 导入，因为对应 job/ledger 不在 Bundle v2；导入后由目标库重新调度。
...
1. **RED**：
   ...
   - 坏引用、畸形 Chronicle 图仍成功导入；
   - 预存损坏 JSON 被 loader 当空集合，导入可能覆盖原盘；
   - Conversation create 持久化失败仍返回对象并污染缓存。
2. **GREEN**：
   ```text
   cargo test -p storyforge --lib campaign_bundle
   # 13 passed
   ```
   覆盖 strict preflight、atomic + 磁盘验证回滚、Chronicle scope/lineage/code/span/graph、covers/covered_by rewrite、pending marker 清理、broken refs、variables/multi-character/provenance、unsupported version 与既有 rewrite/export 测试。
...
cargo test -p storyforge-app-conversation --lib
# 15 passed

cargo test -p storyforge-infra-import --lib
# 27 passed; 1 ignored (real fixture)

cargo test -p storyforge --lib campaign_bundle
# 13 passed

cargo test -p storyforge --lib
# 225 passed; 3 ignored

cargo clippy -p storyforge-app-conversation -p storyforge-domain -p storyforge-infra-import -p storyforge --all-targets -- -D warnings
# Finished ok
...
6. `f4d5a49` fix(import): harden bundle graph rewrite, refs, and rollback verification
7. `90c4d9d` fix(import): fail closed on corrupt bundle state

## 修改文件（主要）

- `crates/domain/src/character.rs` — ST key aliases + resolved helpers
- `crates/domain/src/world_info.rs` — `WorldInfoBook.metadata` 与 entry extra round-trip
- `crates/app-conversation/src/lib.rs` — persist-before-cache Conversation create
- `crates/infra-import/src/lib.rs` — 裸 JSON / PNG 内嵌 JSON BOM strip
...
- `crates/tauri-app/src/lib.rs` — strict preflight、atomic bundle import、Chronicle graph validator + 扩展测试
...
- **原子性粒度**：JSON 文件 store 仍非单文件事务；现已在写前做 strict baseline、失败路径做补偿删除并严格重读磁盘，验证失败会升级为 storage 错误，但并发同 store 写入仍需上层串行化。
