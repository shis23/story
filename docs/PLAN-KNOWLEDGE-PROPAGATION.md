...
> 状态：主体 MVP 已实现（方向 1/2/3 + UI 可解释链路 + 方向 4 传话链 MVP + 方向 5 秘密封口 MVP 已落地）
...
**数据模型和写回链路已有支撑**：
- `KnowledgeSource` 四元分类：`Witnessed`/`ToldByOther`/`Inferred`/`Backstory`。`ToldByOther` 已记 `source_character_id`；若能匹配来源角色已有知识，还会写入 `source_knowledge_id`——**告知来源和传话链均可追溯**。
...
- 可解释读侧已接入：`KnowledgeEntryDto` 现在包含 `character_name`、`source_character_name`、`source_knowledge_id`、`relay_chain_text` 和 `provenance_text`，`CampaignKnowledgeTab.vue` 会优先展示角色名和来源链路，id 仅作为 fallback。
...
| D | 传话链 | A→B→C 跨轮传播 | ✅ MVP 已实现 | `source_knowledge_id` 链接上游知识，UI 展示链路 |
...
- postprocess 在抽取时，若成文里 B 把已知信息告诉 C，仍输出 C 的 `ToldByOther`（source=B）。
...
4. **已完成 MVP：方向 4**（传话链）：`source_knowledge_id` 自动链接匹配来源知识，读侧展示 A→B→C；后续重点是真实 LLM 行为评测、语义匹配和显式知识引用。
...
2. 传话链是否继续扩展到语义相似匹配、显式知识引用 id、Meta 调试解释和真实 LLM 对抗评测？
