...
1. **记忆 / Context 装配规格续作**（与发布矩阵可并行）：`docs/MEMORY-CONTEXT-COMPILER-SPEC-2026-07-11.md` **v1.0** — **M0–M4 可独立部分完成**（含 A/B/C `search/get_chronicle` 预算与工具目录、lineage 回填、Compressor 可恢复 job 队列）。下一步：**M5** 真模型缓存/远楼/压缩损失验收；NarrativeContract/ScenePlan；UnitOfWork/SQLite 仍独立。Quality Error 拦截 + force→Degraded 已接。
