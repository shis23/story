...
1. **记忆 / Context 装配规格续作**（与发布矩阵可并行）：`docs/MEMORY-CONTEXT-COMPILER-SPEC-2026-07-11.md` **v1.0** — **M0–M4 可独立部分完成**（epoch 快照 + Compressor A→B/B→C 后台发布）。下一步：**M5** 真模型缓存/远楼/压缩损失验收。Quality Error 拦截 + force→Degraded 已接。
