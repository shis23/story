...
1. **记忆 / Context 装配规格续作**（与发布矩阵可并行）：`docs/MEMORY-CONTEXT-COMPILER-SPEC-2026-07-11.md` **v1.0** — **M0 完成**、**M1 基本完成**、**M3 最小**（Director `search_chronicle`/`get_chronicle` 对 A）；下一步 **M2 主路径装配接线**（epoch 快照进 MessageLayout、硬去重），再 M4 后台压缩与 B/C。Quality Error 拦截 + force→Degraded 已接。
