...
| H-014 | 同步 fs 阻塞异步运行时 | 🟡 2026-07-07 已完成十二个切片：`start_writing` / `regenerate` 通过 `fill_campaign_context_async` 在 `spawn_blocking` 中加载 active Campaign 与 CampaignRuntimeContext 快照；`start_writing` 的对话准备、legacy 开场白写入与 user intent 追加通过 `prepare_start_conversation_async` offload；成功写作/重 roll 后的临时 instance 落盘通过 `persist_temporary_instances_async` offload；postprocess 写回通过 `persist_postprocess_outcome_async` offload；`accept_variant` 的 `ConversationStore` 写盘通过 `accept_variant_async` offload；`configure_embedder` 的 `embed.json` / SecretRef 写盘通过 `configure_embedder_async` offload；手动/自动归档的 `ConversationStore` 消息读取通过 `archivable_messages_async` offload；连接创建/删除/切换 active 的 `connections.json`、SecretRef 与 `last_used_at` 写盘通过 active connection async helpers offload，并用同一 async mutex 保持磁盘 active_id 与内存 client 一致；`meta_analyze_mvu_card` 的 `mvu_translations.json` 保存通过 `save_mvu_translation_async` offload；`extract_characters` 的 `cards.json` 保存通过 `save_character_card_async` offload；`inspect_generation` 的 `ConversationStore::get` 溯源读取通过 async `GenerationExplainer` + `ConvGenerationExplainer` 的 `spawn_blocking` offload；`meta_classify_st_preset` 的预设读取通过 `load_preset_for_classification_async` offload，避免首次 `PresetStore` 初始化阻塞 async runtime。其他 store 同步写入仍待分批 |
...
1. **H-014 + H-013 剩余项**（性能基础）— 写作/重 roll Campaign 快照读取、start_writing 对话准备、临时 instance 落盘、postprocess 写回、accept variant 写盘、configure embedder 写盘、归档消息读取、连接 active-state 写盘、MVU 翻译保存、角色识别卡保存、inspect_generation 溯源读取与 meta_classify_st_preset 预设读取已 offload；继续评估其他同步写入异步化与 CampaignStore 后台 flush
