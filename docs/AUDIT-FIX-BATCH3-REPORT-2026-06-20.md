...
1. **H-014 + H-013 剩余项**（性能基础）— 写作/重 roll Campaign 快照读取、start_writing 对话准备、临时 instance 落盘、postprocess 写回、accept variant 写盘、configure embedder 写盘、归档消息读取、连接 active-state 写盘、MVU 翻译保存与角色识别卡保存已 offload；继续评估其他同步写入异步化与 CampaignStore 后台 flush
