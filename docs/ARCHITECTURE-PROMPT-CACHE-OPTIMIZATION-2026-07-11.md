...
> | B | **主结构已接** | `NarrativeContract` + `ScenePlan` 扩展；Subagent `current_desire`/`ongoing_action`/`emotion_stage`；Editor/Director 注入；QualityGate：破折号/否后肯 + **显式探针** `PrivateKnowledgeLeak`（SHA-256 指纹，不落原文）；Prompted `builtin-cot-scene-checklist`；Error 拦截 + force→Degraded | 生产探针生成；attribution-aware 私密泄漏硬门禁；有界自动修复（1× Editor）；Editor performance 硬红action；真实 LLM A/B |
...
1. 增加 NarrativeContract。 **（已落地：`domain::narrative_contract`，from_plan_and_runtime + prompt/gate 接线；生产路径 `must_not_reveal` 默认空，仅手填/测试探针触发硬拦）**
2. 扩展 ScenePlan：冲突、对立目标、stakes、beats、complication、must_not_resolve、exit_hook。 **（已挂 `Plan.scene_plan`；emit/parse/Editor tail 已接）**
3. 增加第一版 DraftQualityGate：重复、视角、格式、连续性。 **（Error 拦截 + force→Degraded 已接；Warning 不拦；已扩破折号/否后肯；`PrivateKnowledgeLeak` 仅对显式探针；无自动重写）**
...
**当前诚实口径**：私密归属契约 + 显式探针 Gate 基础机制已接；**不是** attribution-aware 生产私密泄漏门禁已完成。
