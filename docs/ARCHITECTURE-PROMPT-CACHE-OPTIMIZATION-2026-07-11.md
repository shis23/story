...
> | C | **M0–M4.1 可独立部分** | history-epoch + epoch 快照 + prompt catalog；near_raw history 收敛；A/B/C tools；Turn 只计 A；Compressor **claim/防双 worker + metadata heal** | M5 真模型；完整 publication intent 事务日志 |
