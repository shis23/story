...
- 真实复杂卡导入 + Campaign bundle roundtrip 专项 smoke 已补：`scripts/run-real-card-smoke.ps1` 默认读取仓库根目录 `test-card.png`，先运行 Tauri-free ignored 回归 `test_real_complex_card_fixture_preserves_core_st_fields`，再用 `cargo test -p storyforge --lib -- --list` 预检 Tauri lib 测试 harness，预检通过后继续运行 `test_real_complex_card_fixture_can_create_campaign_and_roundtrip_bundle` 和 `test_real_complex_card_offline_mvu_plumbing_smoke`。导入保真回归断言卡名、6 个 alternate greetings、441 条世界书、85 个常驻条目、340 个选择性条目，以及 `regex_scripts`、`tavern_helper`、`xiaobaix-template` 等关键 extensions 保留；Campaign/bundle 回归把同一真实卡保存为 `CharacterCard`，创建 Campaign/instances，导出 StoryForge bundle，再导入新 store 并断言关键 extensions、6 个 alternate greetings 和 instances 仍保留。若当前 Windows 环境触发 `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)`，默认脚本会失败并给出 loader 诊断；追加 `-SkipTauriOnLoaderError` 只会跳过 Tauri-only 步骤并完成导入保真局部 smoke，不替代完整 S1、UI 真实操作和真实写作验收。
- B5 Meta 确定性后端 smoke 已补：`scripts/run-meta-smoke.ps1` 依次运行生成溯源解释、Campaign health repair proposal、typed patch preview+accept、typed patch dismiss 四个后端回归；覆盖 patch 先 preview、accept 写盘、dismiss 不进入 pending 的命令层契约。该脚本不替代桌面 MetaPanel 截图、真实 trace/provenance UI 展示和 Campaign bundle 人工验收。
...
> **2026-09-01 缺陷修复复验收口**：上一轮 GUI 验收发现的 3 个缺陷 + 复验中新发现的 3 个缺陷全部修复并以真实 LLM 复验：①plugin-fs v1 死 API `writeBinaryFile`（v2 为 `writeFile`）——4 处导出全链路静默失败（B6/S1 根因）；②`readTextFile/writeTextFile` 映射的 fs 命令未在 capability 授权——导入 Bundle/预设/Agent Profile 被 scope 拒绝（S1 roundtrip 根因），统一改 `readFile`+TextDecoder 并以 capabilities 静态测试锁定两类死用法；③重启后 active Campaign 不恢复（SQLite 指针仅内存为既定设计）——AppV2 启动无 active 时从最近会话恢复；④`set_active_campaign` 不同步 campaign runtime 快照——重启/切换后未写作前 Meta 报「当前没有 active Campaign」，现锁前预加载快照、after_commit 写入 tool_ctx；⑤HealthCheckPanel 补丁卡片渲染顺序——体检无问题时 agent_proposed typed patch 被 v-if 分支吞掉，且挂载不拉取 pending；⑥生成失败完全无 UI 反馈——WritingScreen 新增错误提示条 + errorText 统一结构化错误文案（~50 处路径）。复验结果：B4/B5/B6/S1/S5/L6 全部按预期通过（见各行），B2 维持部分通过（写回语义已由 B4 证明，B2 三轮 Campaign 的轮次状态为验收工具误触历史遗留）。证据根 `artifacts/fe-acceptance-2026-08-31/`（本机，不入库，密钥扫描清洁）。
>
...
| --- | --- | --- | --- | --- | --- |
| B1 单角色首轮 | 一张 ST V2/V3 单角色 PNG 或 JSON；一个可用 LLM 连接；空白或临时数据目录 | 1. 启动桌面端。<br>2. 导入角色卡。<br>3. 确认角色列表刷新。<br>4. 从该角色创建 Campaign。<br>5. 设为 active Campaign 并写第一轮。 | 导入成功；Campaign 创建成功；Director、Subagent、Editor trace 可见；正文落到当前 Campaign；没有 legacy character 写作路径被误用。 | app 日志；pipeline trace；`log_export_bundle`；导入失败时保留原始卡文件名和错误提示截图。 | GUI 通过 2026-08-31（真实桌面端 + 真实 LLM GLM-4.7：Seraphina 导入→卡库刷新→B1-Seraphina-Acceptance Campaign→首轮真实写作，创作过程回顾 4/4 步（正文续写/规则检查通过/摘要 164 字/记账 知识0·变量1·任务0），总结/变量面板可见，Campaign 面板 tab 可见，无 legacy 路径误用；证据 `artifacts/fe-acceptance-2026-08-31/screenshots/00-16`，隔离 APPDATA 数据目录） |
| B2 多角色三轮 | 一张能抽取多个角色定义的 ST 卡或 JSON；同一数据目录保留三轮写作结果 | 1. 导入多角色卡。<br>2. 抽取 definitions。<br>3. 创建含多个 instance 的 Campaign。<br>4. 连续写 T1/T2/T3。<br>5. 每轮后打开 Pipeline 和 Campaign 面板。 | Director 能看到多个 instances；Subagent 分角色输出；Editor 输出正文；三轮后 summaries、knowledge、variables、tasks 至少有一类影响下一轮。 | app 日志；pipeline trace；Campaign bundle；若某轮失败，记录失败轮次和 Agent 阶段。 | GUI 部分通过 2026-08-31（真实 LLM：multi-b2 卡识别 6 定义→B2-Multi-3Turns 6 实例→顺序剧组模式连写 T1/T2/T3（正文合计约 1.4k 字/轮，T1 创作回顾 9/9 步含「场景规划·分配 4 个角色」+ 吴雨桐/莫挽舟/苏沁/王曼离 分角色子代理 trace）；写回记账当时被前端转发缺陷阻塞（见 B4 行注），轮次状态 failed/awaiting 由该缺陷与验收工具误触所致；证据 screenshots/17-33） |
| B3 同名隔离 | 两个同 display name 但不同 instance id 的角色，或能创建同名 instance 的测试 Campaign | 1. 创建两个同名 instance。<br>2. 给其中一个角色制造私有知识或变量。<br>3. 写一轮让 postprocess 写回。<br>4. 查看 knowledge 和 variables tab。 | 知识、变量按 instance id 落盘；同名角色不串写；name 匹配歧义时不会静默写错目标。 | app 日志；Campaign bundle；knowledge/variables 面板截图；相关 pipeline trace。 | GUI 通过 2026-08-31（按当前产品决策验收：GUI「添加角色」表单与后端 `add_campaign_instance` 均明确拒绝同名实例——「本局已有同名角色『苏沁』」，即 9602476 防串写决策在 GUI 层生效，同名场景在当前产品不可达；知识/变量按 instance id 归属已由 B4 写回证明——知识 2 条挂在苏沁实例 id、变量区分 Campaign 级与实例级；自动化同名不串写测试维持有效） |
| B4 后处理写回 | B2 的三轮 Campaign，或一张明确会产生知识、变量、任务、摘要的测试卡 | 1. 写作后等待 postprocess 完成。<br>2. 打开 summaries、knowledge、variables、tasks。<br>3. 检查 A→B→C 传话链示例。<br>4. 重启 app 后再次查看。 | 本轮摘要出现；知识面板显示知道者、来源者和 provenance；传话链可显示；变量落在正确 instance 或 campaign；任务创建/完成/放弃可见；重启后 active Campaign 和状态恢复。 | app 日志；`log_export_bundle`；Campaign bundle；重启前后截图。 GUI 通过 2026-08-31；重启恢复缺陷已于 2026-09-01 修复并复验（真实 LLM：B4-Writeback-Test 一轮→「采纳」记账小票 9 项→质量门禁 Error 拦截→原生强制采纳→Degraded 落库；SQLite 断言 mutation_commits=1、round_summaries=1、knowledge=2、tasks=1；四类 tab 显示写回内容；第二轮采纳复验 mutation_commits=2、knowledge=6。重启后 active Campaign 不恢复的缺陷已修复（AppV2 启动无 active 时从最近会话恢复 + set_active_campaign 同步 runtime 快照）：复验重启即自动恢复活动+会话正文+待采纳轮（7,437 字），无需手动打开。传话链 A→B→C 专项未在 GUI 展开（自动化已覆盖语义）。证据 screenshots/36-47 + r0/r3/r5） |
| B5 Meta 解释与修复 | 已有至少一轮生成、trace 和 provenance 的 Campaign；一个可触发 health issue 的小问题 | 1. 打开 Meta 面板。<br>2. 运行 health check。<br>3. 查看”解释本轮生成”。<br>4. 生成 patch preview。<br>5. 分别验证 dismiss 和 accept。<br>自动子项：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-meta-smoke.ps1`。 | 解释能引用真实 trace/provenance；patch 先 preview，不直接写核心数据；dismiss 不改状态；accept 后对应 tab 刷新。 | Meta 面板截图；app 日志；patch preview 内容；Campaign bundle；meta smoke 输出。 GUI 通过 2026-09-01（真实 LLM GLM-4.7 全闭环：五 tab 可达；健康检查「未发现问题/数据完整性校验通过」；「解释上一条生成」显示场景/Seed/执笔者/Trace 入口 + Reasoning 脱敏警示；Meta 对话请求修改变量→agent 正确识别 active Campaign 并提议 typed patch（weather 晴转多云→大雨）→健康检查 tab 补丁卡片带 diff 预览（Before/After，不直接写核心数据）→「忽略」后变量不变（仍 晴转多云）、卡片清空→再提议→「接受」后 weather=大雨、pending 清零、变量面板值同步刷新。修复三个阻塞缺陷：①set_active_campaign 现同步 campaign runtime 快照进 tool_ctx（此前重启/切换后未写作前 Meta 一律「当前没有 active Campaign」）②HealthCheckPanel 补丁卡片渲染顺序（体检无问题时 agent_proposed patch 被 v-if 吞掉）+ 挂载即拉取 pending patch。typed patch 不跨重启持久（后端内存态）为当前设计边界。证据 r6-r11 截图） |
| B6 排障与恢复 | 任一已完成 B1-B5 的 Campaign；一次人为制造的失败场景，如断网或取消生成 | 1. 触发失败。<br>2. 确认 UI 或日志可见错误。<br>3. 导出排障 bundle。<br>4. 重启 app。 | 失败不会伪装成成功；active state 不损坏；导出包包含 app/platform、data/log/conversation 路径和 store 摘要；不泄露真实 API key。 | `log_export_bundle`；app 日志；失败时 UI 文案截图。 GUI 通过 2026-09-01（日志 tab 结构化日志落盘 appdata/logs/*.jsonl；**导出 bundle 缺陷已修复并复验**：LogPanel「导出 bundle」→原生另存为→文件落盘 43KB×3（含 diagnostic_context：app/platform/data·log·conversation 路径/23 个 store 文件摘要；regex 扫描无 API key 泄漏）→UI 显示「已导出日志 bundle」成功反馈（6s 自动清除）；根因为 plugin-fs v1 死 API writeBinaryFile（v2 改名 writeFile，解构 undefined 调用即 TypeError 被静默吞掉），修复覆盖 LogPanel/CampaignPanel/AgentProfileManager/PromptHookAuditLog 四处并以 capabilities 静态测试锁定。失败形态 UI 文案：屏障拒绝显示「当前有未完成的轮次…请先 Accept、Discard 或 Abandon」（错误提示条）；取消生成：生成中点「停止生成」→生成停止、无假成功、turn_attempts 无脏记录；多次重启后数据目录一致无损坏、active+会话自动恢复。证据 r1-r4/r13 + exports/b6-log-bundle*.json） |
...
| ID | 输入材料 | 操作步骤 | 预期结果 | 失败日志 / 导出包 | 状态 |
| --- | --- | --- | --- | --- | --- |
| S1 导入保真 | 至少一张复杂 ST/MVU PNG 或 JSON，包含世界书、开场白、标签、extensions 和 raw JSON；自动子项可用仓库根目录 `test-card.png` | 1. 先运行 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-real-card-smoke.ps1`；若本机 Tauri lib harness 因 `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` 无法启动，可用 `-SkipTauriOnLoaderError` 记录 Tauri-free 局部 smoke，但仍需在可启动 Tauri harness 的环境补跑完整脚本。<br>2. 在 UI 导入卡。<br>3. 打开角色详情。<br>4. 创建 Campaign。<br>5. 导出 StoryForge Campaign bundle。 | PNG/JSON 导入成功；世界书、开场白、标签、extensions 不丢；`raw_card_json` 保底保留；StoryForge bundle 可导出并可重新导入。 | 导入错误日志；导出的 Campaign bundle；角色详情截图；real-card smoke 输出。 GUI 通过 2026-08-31 导入保真 + 2026-09-01 roundtrip 补全（UI 导入 test-card.png（命定之诗与黄昏之歌v4.1，6MB/441 世界书）：alternate_greetings=6、世界书 441 条、extensions 7 键全保留、raw_card_json 保留、LLM 识别 7 角色定义。Campaign bundle 导出/重导入 roundtrip 已于 2026-09-01 GUI 通过：B4-Writeback-Test 导出 134KB bundle（campaign+card+definitions+instances 6+knowledge 6+tasks 1+summaries 2，含 weather=大雨）→重导入「Bundle 导入完成」→活动列表出现导入副本；此前导入被 fs.read_text_file 未授权拒绝的缺陷已修（统一 readFile+TextDecoder，capabilities 静态测试锁定）。real-card smoke 自动子项维持既有通过记录。证据 screenshots/76 + exports/26-27 + r12/s5-*） |
...
| S5 导出兼容 | S1-S4 生成的 Campaign；至少一条知识和一条变量 | 1. 导出 StoryForge JSON bundle。<br>2. 导出 ST 卡 PNG 或共享 lorebook（若入口可用）。<br>3. 重新导入导出物做冒烟检查。 | StoryForge 内部多角色 Campaign 不强行退化成单角色卡；导出物保留必要 Campaign 数据；ST 兼容导出说明降级边界。 | 导出 bundle；重新导入日志；导出文件名和大小记录。 GUI 通过 2026-09-01（StoryForge JSON bundle 导出 134KB（campaign/card/definitions/instances/knowledge/tasks/summaries 全保留，多角色 Campaign 未退化单角色）→重导入冒烟通过（导入副本出现在活动列表，关联会话可见）；ST 卡 PNG 导出通过：合法 PNG（24.7KB）含 chara tEXt chunk，按实例导出；共享 lorebook 入口本轮未触发（campaign 无共享世界书数据）。文件名/大小记录：exports/s5-campaign-bundle.json、exports/s5-st-card.png。证据 r12 + exports/s5-*） |
...
| L6 Postprocess 覆盖 | L3 的三轮 Campaign；至少一轮明确包含事实、变量变化、任务变化和摘要点 | 1. 写作后等待 postprocess。<br>2. 检查 summaries、knowledge、variables、tasks。<br>3. 记录未命中的类别。 | 知识、变量、任务、摘要至少各命中一次；Postprocess 不凭空创建永久事实；失败重试不无限循环。 | app 日志；Campaign bundle；面板截图；成本记录。 通过 2026-09-01（真实 LLM GLM-4.7，B4-Writeback-Test 两轮采纳：纪要/知识/变量/任务四类各至少命中一次——首轮小票 9 项（纪要1/知识2/变量5/任务1），次轮 knowledge 2→6；mutation_commits 1→2、round_summaries 1→2；未观察到凭空创建永久事实；失败重试无无限循环（屏障拒绝文案可观察）。成本记录：每轮正文 1 次 + 廉价记账 2 次调用，见 appdata/logs JSONL token 统计。证据 r5 + SQLite 断言） |
| L7 M5 endurance | 固定真实模型；production `H_anchor=5` / `E=10`；独立脱敏 evidence 目录 | 1. 设置真实模型 env 与预算。<br>2. 依次通过 canary/coverage/stability gate。<br>3. 运行 full 100 Accept。<br>4. 检查 checkpoint、usage、epoch、early-fact 与 secret scan。 | 100/100 Accept；预算不超限；epoch rollover 与 near_raw 截断正确；证据无 key/prompt/story 正文；路径字段准确声明 production/synthetic 边界。 | `endurance_calls/turns/checkpoint/manifest.jsonl`；RESULT；不得提交原始 key。 | 部分通过（Gate 6，RESULT §35）：Canary3 / Coverage12 / TextFallback3 / Stability30 已 PASS 并 seal；**Full100 未完成**——4 次重跑受 relay 间歇不稳定阻断，r3 跑到 turn 58/100 全健康（564 次真实调用，§35.8 深度分析）。**Gate 6 已于 2026-08-31 按用户决议关闭（非 PASS，RESULT §35.9）：不再续跑 Full100 作为前置；如需 100-turn 长程证据另行立项、另记新证。**旧 45/100（2026-07-14）是历史 Partial Evidence，原始 JSONL 已清理，不复活。 |
| L8 完整生产 Postprocess | 已抽出的共享 ProductionPostprocessService；真实模型；可审计 Campaign | 1. 由 Tauri 与 harness 调用同一共享服务。<br>2. 跑 Summarizer、PostProcessor、Attempt 同步、Chronicle A、Accept。<br>3. 注入迟到结果、取消和存储失败。 | 无 synthetic Chronicle；Attempt/hash/quality 与最终稿一致；迟到结果不复活；失败传播；Accept 后摘要/知识/变量/任务和索引一致。 | call/turn evidence；Campaign/SQLite 导出；故障注入日志。 | 部分通过（Gate 6，RESULT §35）：共享服务已在 Gate 2/3 抽出，Tauri 与 harness 共用；Stability30/Canary3 等 seal 证据覆盖真实 Summarizer/PostProcessor/Attempt 同步；Full100 完整终态（含迟到结果/取消注入的 100-turn 观测）未取得——Gate 6 已关闭（2026-08-31，非 PASS，RESULT §35.9），如需该观测另行立项补证 |
...
- 真实卡 Gold 档兼容尚未完成验收。
- **存储默认已切换为 SQLite（Gate 7，2026-08-05，RESULT §36）**：发布候选必须保留至少一个发布周期的 JSON importer、SQLite→JSON reverse export、迁移备份（sqlite-backups/）与显式紧急回退（`STORYFORGE_STORAGE_BACKEND=json`）；发布后收集完整候选周期统计（自动回退率/迁移失败率），稳定期后另立计划删除 JSON 生产写路径。候选构建需抽样验证旧 JSON 用户自动迁移的成功提示/失败提示可见性。
