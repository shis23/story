...
| B4 后处理写回 | B2 的三轮 Campaign，或一张明确会产生知识、变量、任务、摘要的测试卡 | 1. 写作后等待 postprocess 完成。<br>2. 打开 summaries、knowledge、variables、tasks。<br>3. 检查 A→B→C 传话链示例。<br>4. 重启 app 后再次查看。 | 本轮摘要出现；知识面板显示知道者、来源者和 provenance；传话链可显示；变量落在正确 instance 或 campaign；任务创建/完成/放弃可见；重启后 active Campaign 和状态恢复。 | app 日志；`log_export_bundle`；Campaign bundle；重启前后截图。 | GUI 通过 2026-08-31（真实 LLM：B4-Writeback-Test 一轮→修复后「采纳」弹出记账小票 9 项（纪要 1/知识 2/变量 5：story_clock·weather·location·mood·state/任务 1）→质量门禁 Error 拦截→原生强制采纳确认→Degraded 落库；SQLite 断言 mutation_commits=1、round_summaries=1、knowledge=2、tasks=1；四类 tab 面板均显示写回内容（变量区分 Campaign/实例级、知识带来源轮次）；重启后会话内容/字数/已采纳状态恢复。注：修复前消息级采纳因 WritingScreen 转发缺陷（commit `6a0fbd8`）全链路失效，本项在修复后验证通过；另发现重启后 active Campaign 不自动恢复、需手动打开会话或重设。传话链 A→B→C 专项未在本轮 GUI 展开（自动化已覆盖语义）。证据 screenshots/36-47） |
| B5 Meta 解释与修复 | 已有至少一轮生成、trace 和 provenance 的 Campaign；一个可触发 health issue 的小问题 | 1. 打开 Meta 面板。<br>2. 运行 health check。<br>3. 查看”解释本轮生成”。<br>4. 生成 patch preview。<br>5. 分别验证 dismiss 和 accept。<br>自动子项：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-meta-smoke.ps1`。 | 解释能引用真实 trace/provenance；patch 先 preview，不直接写核心数据；dismiss 不改状态；accept 后对应 tab 刷新。 | Meta 面板截图；app 日志；patch preview 内容；Campaign bundle；meta smoke 输出。 | GUI 部分通过 2026-08-31（面板五 tab 可达；健康检查对 B4 返回「未发现问题」；「解释上一条生成」显示场景/Seed/执笔者/原始 Trace 入口并带 Reasoning 脱敏警示；Patch tab 空态文案正确。未完成：patch preview/dismiss/accept 的 GUI 闭环——Meta 对话请求修改变量时代理回答「当前没有 active Campaign」而拒绝提议（重启后后端侧 active 上下文未恢复所致，见 B4 行注）；该闭环继续由 meta smoke 4/4 自动化背书。证据 screenshots/52-70） |
| B6 排障与恢复 | 任一已完成 B1-B5 的 Campaign；一次人为制造的失败场景，如断网或取消生成 | 1. 触发失败。<br>2. 确认 UI 或日志可见错误。<br>3. 导出排障 bundle。<br>4. 重启 app。 | 失败不会伪装成成功；active state 不损坏；导出包包含 app/platform、data/log/conversation 路径和 store 摘要；不泄露真实 API key。 | `log_export_bundle`；app 日志；失败时 UI 文案截图。 | GUI 部分通过 2026-08-31（日志 tab 可见结构化日志且 appdata/logs/2026-08-31.jsonl 落盘；多次重启后数据目录一致、无损坏、会话完整恢复。**发现缺陷：排障 bundle 导出静默失败**——LogPanel「导出 bundle」三次触发，原生另存为对话框正常、用户确认保存后目标文件始终未写入磁盘且无任何成功/失败提示；需修复后重跑。失败场景的 UI 文案（断网/取消）未在本轮单独制造。证据 screenshots/71-73） |
...
| ID | 输入材料 | 操作步骤 | 预期结果 | 失败日志 / 导出包 | 状态 |
| --- | --- | --- | --- | --- | --- |
| S1 导入保真 | 至少一张复杂 ST/MVU PNG 或 JSON，包含世界书、开场白、标签、extensions 和 raw JSON；自动子项可用仓库根目录 `test-card.png` | 1. 先运行 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-real-card-smoke.ps1`；若本机 Tauri lib harness 因 `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` 无法启动，可用 `-SkipTauriOnLoaderError` 记录 Tauri-free 局部 smoke，但仍需在可启动 Tauri harness 的环境补跑完整脚本。<br>2. 在 UI 导入卡。<br>3. 打开角色详情。<br>4. 创建 Campaign。<br>5. 导出 StoryForge Campaign bundle。 | PNG/JSON 导入成功；世界书、开场白、标签、extensions 不丢；`raw_card_json` 保底保留；StoryForge bundle 可导出并可重新导入。 | 导入错误日志；导出的 Campaign bundle；角色详情截图；real-card smoke 输出。 | GUI 导入保真通过 2026-08-31（真实桌面端 UI 导入 test-card.png（命定之诗与黄昏之歌v4.1，6MB/441 世界书）：SQLite 断言 alternate_greetings=6、世界书 441 条、extensions 7 键全保留（含 regex_scripts=13、tavern_helper、xiaobaix-template）、raw_card_json 保留；LLM 识别完成 7 个角色定义，卡库三卡全显示。未完成：Campaign bundle 导出/重导入 roundtrip 未跑——与 B6 同一导出静默失败缺陷相关，待修复后重跑；real-card smoke 自动子项维持既有通过记录。证据 screenshots/76 + exports/26-27） |
