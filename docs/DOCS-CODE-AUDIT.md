...
2026-07-07 真实复杂卡导入核对：`cargo run -p storyforge-infra-import --example inspect_card` 已用仓库根目录 `test-card.png`（命定之诗与黄昏之歌 v4.1）解析通过；该卡包含 441 条世界书、6 个 alternate greetings，并保留 `regex_scripts`、`tavern_helper`、`xiaobaix-template` 等 extensions。已补 `scripts/run-real-card-smoke.ps1` 和 ignored 回归 `test_real_complex_card_fixture_preserves_core_st_fields` / `test_real_complex_card_fixture_can_create_campaign_and_roundtrip_bundle`，把复杂卡导入、保存为 `CharacterCard`、创建 Campaign/instances、StoryForge bundle 导出和新 store 导入后的关键 extensions / alternate greetings / instances 保真固化为可复跑 smoke；此检查证明复杂卡 import/inspect 与 Campaign bundle roundtrip 基线可跑，但不等同完整 UI 真实操作、真实写作或插件运行时验收。

2026-07-07 续补核对：在本机运行 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-real-card-smoke.ps1` 已完整通过，未触发 Tauri loader 降级路径；覆盖 `test_real_complex_card_fixture_preserves_core_st_fields`、`test_real_complex_card_fixture_can_create_campaign_and_roundtrip_bundle`、`test_real_complex_card_offline_mvu_plumbing_smoke` 三段，证明当前 Windows 环境可复跑真实复杂卡导入保真、Campaign bundle roundtrip 和离线 MVU plumbing 自动 smoke。仍未替代 UI 真实导入/截图、真实 LLM 写作和插件运行时验收。
