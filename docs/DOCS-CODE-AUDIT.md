...
2026-07-07 增量核对：`storyforge-infra-regex` 的执行器已支持 `minDepth/maxDepth` 包含式范围过滤。`app-pipeline` 当前生成/重 roll 的 Input/Output 按 ST depth 0 执行；Tauri `get_conversation` 生成 `display_content` 时按消息节点离末尾的深度执行 display-only 正则，因此老消息不会被只针对最近 N 条的脚本误替换。prompt 历史批量重写和完整 Slash 命令系统仍未接线；Regex Slash placement 3 的 `/` 前缀输入最小 hook 见下方追加核对。
