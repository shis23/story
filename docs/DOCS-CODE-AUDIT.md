...
- `crates/tauri-app/src/campaign_store.rs::pressure_sync_json_io_across_collections` **已补同步 JSON I/O ignored 压测**：4 集合并发写入并记录 p50/p95/max；本机 500 次/集合通过，p95 < 9ms、max 约 28ms。2026-07-07 已先把写作/重 roll 的 Campaign 快照读取移入 `spawn_blocking`，并把临时 instance 落盘移入 `persist_temporary_instances_async`、postprocess summary/knowledge/variable/task 写回移入 `persist_postprocess_outcome_async`、对话变体采纳写盘移入 `accept_variant_async`，降低 async Tauri 命令上的同步读写阻塞；桌面小/中等数据量暂不阻塞，Android/真实长会话、其他同步写入和后台 flush 仍需复测。
