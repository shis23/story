...
- **修复**: 已完成集合级锁拆分，并增加并发写回回放测试覆盖跨集合写入后重载一致性。2026-07-07 已将写作/重 roll 入口的 Campaign 快照读取搬到 `spawn_blocking`；同日新增 postprocess summary/knowledge/variable/task 写回 `spawn_blocking` 切片。剩余工作是继续压测同步 JSON 写入，并评估临时 instance 落盘、其他命令写入、后台 flush / 更广泛 `spawn_blocking`。
- **置信度**: R5 单次确认 + 并发回放测试
...
- **当前状态（2026-07-07）**: `start_writing` / `regenerate` 已改用 `fill_campaign_context_async`，在清空旧 runtime 后通过 `tokio::task::spawn_blocking` 加载 active Campaign 与 CampaignRuntimeContext 快照，再回到 async 主线写入 `WritingContext` / `ToolContext`。成功写作/重 roll 后的 postprocess summary/knowledge/variable/task 写回也已通过 `persist_postprocess_outcome_async` offload 到 blocking pool，保留 `persist_postprocess_outcome_to_store` 作为可测同步核心。新增 `test_campaign_context_snapshot_applies_runtime_to_contexts` 与 `test_postprocess_persistence_helper_writes_all_campaign_outputs` 覆盖关键行为。剩余为临时 instance 落盘、其他 store 写入与后台 flush 评估。
- **置信度**: R5 单次确认
...
15. **H-014**: 全量同步 I/O 替换为异步（写作/重 roll Campaign 快照读取、postprocess 写回已先行 `spawn_blocking`）
16. **H-013 剩余项**: CampaignStore 后台 flush / 异步写入评估（集合级锁、快照读取 offload、postprocess 写回 offload 已完成）
