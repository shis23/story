...
15. **H-014**: 全量同步 I/O 替换为异步（写作/重 roll Campaign 快照读取、临时 instance 落盘、postprocess 写回、accept variant 写盘、configure embedder 写盘、归档消息读取、连接 active-state 写盘、MVU 翻译保存已先行 `spawn_blocking`）
